...
inline vec3 pow(const vec3& u, const vec3& v)
{
	return vec3(std::pow(u.e[0], v.e[0]), std::pow(u.e[1], v.e[1]), std::pow(u.e[2], v.e[2]));
}
...
// uniform: Lambertian distribution
...
{
	double cos_theta = fmin(dot(-v, n), 1.0f);
